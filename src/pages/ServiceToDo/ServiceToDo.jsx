...
const ServiceToDo = () => {
    ...
    useEffect(()=>{
        axios(`http://localhost:5000/booked?email=${user?.email}`)
        .then(data=>{
   ...
   services?.map((service, idx)=>
               <tr key={service._id} className={`hover:bg-emerald-200 rounded-lg text-center my-2 ${
                ...
                'text-pink-500 bg-pink-100/60'
                   ...
                   <td><select className="bg-transparent" value={service.status} onChange={(e)=>handleStatus(service._id, e.target.value)} name="status" id="status">
